...
class PublicKeyList extends React.Component {
  ...
  componentDidMount() {
    const {
      ...
      contactId,
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'requestPublicKeys' does not exist on typ... Remove this comment to see the full error message
      requestPublicKeys,
      ...
      isFetching,
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'needsFetching' does not exist on type 'R... Remove this comment to see the full error message
      needsFetching,
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'didInvalidate' does not exist on type 'R... Remove this comment to see the full error message
      didInvalidate,
    } = this.props;
  ...
  getPublicKeyDataUrl = ({ key }) =>
    `data:application/x-pgp;base64,${strToBase64(key)}`;

  handleEdit = (publicKey) => () =>
    this.setState({ editMode: true, editKey: publicKey.key_id });
      ...
      <li key={publicKey.key_id} className="m-public-key-list__key">
        <Icon
          ...
          className={`m-public-key-list__quality-icon--${
            KEY_QUALITY_CLASSES[this.getKeyQuality(publicKey)]
          }`}
          rightSpaced
        />
        ...
        <Link
          ...
          title={i18n._('contact.public_key_list.download_key', null, {
        ...
        <Button
          ...
          onClick={this.handleEdit(publicKey)}
        />
      </li>
    );
  };
  ...
  render() {
    // @ts-expect-error ts-migrate(2339) FIXME: Property 'publicKeys' does not exist on type 'Read... Remove this comment to see the full error message
    const { publicKeys } = this.props;

    return (
      <>
        <ul>{publicKeys.map((publicKey) => this.renderKeyItem(publicKey))}</ul>
        {this.renderAddForm()}
      </>
    );
  }
