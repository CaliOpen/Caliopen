...
const updateMessagesTagsConcrete =
  (i18n, messageIds, tags) => async (dispatch, getState) => {
    const {
    ...
    const updateTagsColl = (message) => {
      const tagsToSave = (
        message.tags ? getCleanedTagCollection(userTags, message.tags) : []
      )
...
export const updateMessagesTags =
  (i18n, messageIds, tags, { withThrottle = true } = {}) =>
  (dispatch) =>
