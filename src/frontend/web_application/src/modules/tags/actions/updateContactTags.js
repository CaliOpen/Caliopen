...
const updateContactTagsConcrete =
  (i18n, contactIds, tags) => async (dispatch, getState) => {
    const {
    ...
    const updateTagsColl = (contact) => {
      const tagsToSave = (
        contact.tags ? getCleanedTagCollection(userTags, contact.tags) : []
      )
...
export const updateContactTags =
  (i18n, contactIds, tags, { withThrottle = true } = {}) =>
  (dispatch) =>
